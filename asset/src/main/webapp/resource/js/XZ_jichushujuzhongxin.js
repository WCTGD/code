...
function hh() {
	/*
	 * $('.fh_zj').click(function(){ $('.main_div').append('<input
	 * class="main_text" type="text"/>') }) $('.fh_bc').click(function(){
	 * $('.main_text').remove(); })
	 */
	/*
	 * $('.fh_zj').click(function(){ $('.main_div
...
$('#xgys').click(function() {
	$('.xgys_yy').eq(0).show();
	$('.xz_save').click(function() {
		$('.xgys_yy').hide();
	})
})
$('.xz_close_img').click(function() {
	$('.xgys_yy').hide();
})
// 新承包商/修改
$('#xcbs').click(function() {
...
function decide() {
	...
	var SecurityGuidance = $('#sgId').val();
	if (no == "" && zcId2 == "" && workerId == "" && positionIds == "" && sws == "" && information == "" && informationB == "" && saw == "" && sjp == "" && sul == "" && troc == "" && delectAc == "" && supplierOn == "" && BaseContractorNo == "" && SecurityGuidance == "") {
		alert("请选中行!");
		$('.sc_yy').hide();
		return;
	}
...
function skip() {
	...
	var b = $("#totalPage").html();
	if (skipId <= b && skipId != 0 && !isNaN(b) && skipId > 0) {
...
//			// w.no+"</div></td><td><div
//			// class='main_div'>"+w.name+"</div></td></tr>"
//			// }
...
// var str = "";
// str += "<tr class='data' onclick=clickAssetClasses(this)
// id='"+basicData[i].no+"'>";
// str += "<td><div class='main_div'><input type='text' readonly
// value='"+basicData[i].no+"'/>"+basicData[i].no+"</div></td>";
// str += "<td><div class='main_div'
// value='"+basicData[i].assetCategory+"'><input type='text' readonly
// />"+basicData[i].assetCategory+"</div></td>";
// str += "</tr>";
// $("#tb_ac").append(str);
// }
// if(basicData.length< Number(pagesize)){
// for(var j=0; j<(pagesize-basicData.length);j++){
// var str = "<tr class='acId'><td><div class='main_div'><input type='text'
// name='saveAc' readonly/></div></td><td><div class='main_div'><input
// type='text' name='saveAcName' readonly /></div></td></tr>";
// $("#tb_ac").append(str);
// }
// }
// }
...
function form_add_Supplier() {
	...
	if(supplierName == null || supplierName == "" || contactName == null || contactName == "" || telphone == null || telphone == "" || service == null || service == ""){
		alert("不能为空！");
		$('.xgys_yy').show();
	}else{
...
$(function() {
	$("#yes").click(function() {
		...
		if(assetCategory == null ||  assetCategory == undefined || assetCategory == ""){
			return;
		}
		$.ajax({
			url : pathName + "/basicData/deleteAssetClasses",
...
function click2(t) {
	...
	$(t).attr("style", "background-color: #ffff99");
	$('#zcId2').val("");
	$('#zcId2').val($(t).children("td").children('div').children('input').val());
}
// 删除资产
...
function upload1() {
	$.ajaxFileUpload({
		type : "post",
		url : pathName + "/basicData/uploadFile", // 用于文件上传的服务器端请求地址
...
$("#aqzd_qx").click(function(){
	SecurityGuidance();
})

// 安全指导
function SecurityGuidance() {
	var pageCode = $("#pageCode_sg").text();
	var pagesize = $("#pageSize_sg").val();
	$.ajax({
		url : pathName + "/basicData/listUploadFile",
		type : "post",
		dataType : "json",
		contentType : "application/json;charset=UTF-8",
		data : JSON.stringify({
		...
		success : function(data) {
			var p = data.page;
			$("#pageCode_sg").html(p.pageCode);
			$("#totalRecord_sg").html(p.totalRecord);
			$("#start_sg").html(p.start + 1);
			$("#end_sg").html(p.end);
			$("#totalPage_sg").html(p.totalPage);
			var uploadFile = data.uploadFile;
			$("#tb_sg").empty();
			for (var i = 0; i < uploadFile.length; i++) {
				var str = "";
				str += "<tr class='data' onclick=click_sg(this) id='" + uploadFile[i].no + "'>";
				str += "<td ><div class='main_div' ><input type='text' readonly value='" + uploadFile[i].no + "'/>" + uploadFile[i].no + "</div></td>";
				str += "<td><div class='main_div' value='" + uploadFile[i].name + "'><input type='text' readonly />" + uploadFile[i].name + "</div></td>";
				str += "</tr>";
				$("#tb_sg").append(str);
			}
			if (uploadFile.length < Number(pagesize)) {
				$('#bm').val(pagesize - uploadFile.length);
				for (var i = 0; i < (pagesize - uploadFile.length); i++) {
					var str = "<tr class='trId'><td><div class='main_div'><input type='text' name='saveId' readonly/></div></td><td><div class='main_div'><input type='text' name='saveName' readonly /></div></td></tr>";
					$("#tb_sg").append(str);
				}
			}
...
// 前一页(安全指导查询)
function before_sg() {
	var a = $("#pageCode_sg").html();
	if (a > 1) {
		$("#pageCode_sg").html(a - 1);
		SecurityGuidance();
	} else {
		alert("已经是第一页了")
	}
...
// 后一页(安全指导查询)
function after_sg() {
	var b = $("#totalPage_sg").html();
	var a = $("#pageCode_sg").html();
	if (a < b) {
		$("#pageCode_sg").html(Number(a) + 1);
		SecurityGuidance();
	} else {
		alert("已经是最后一页了");
	}
...
// 跳转(安全指导)
function skip_sg() {
	var skipId = $("#skipId_sg").val();
	var b = $("#totalPage_sg").html();
	if (skipId <= b && skipId != 0 && !isNaN(b) && skipId > 0) {
		$("#pageCode_sg").html(skipId);
		SecurityGuidance();
	} else {
		alert("不存在！");
		$('#skipId_sg').val("");
...
// 每页显示的下拉框(安全指导)
function strike_sg() {
	var size = $("#pageSize_sg").val();
	$("#pageSize_sg").val(size);
	SecurityGuidance();
}
...
function click_sg(t) {
	...
	$(t).attr("style", "background-color: #ffff99");
	$('#sgId').val("");
	$('#sgId').val($(t).children("td").children('div').children('input').val());
}

// 安全指导 删除
function deleteSecurityGuidance() {
	$('.sc_yy').show(); // 从display:none还原元素默认或已设置的display属性
}
...
$(function() {
	$("#yes").click(function() {
		var no = $('#sgId').val();
		$.ajax({
			url : pathName + "/basicData/deleteSecurityGuidance",
			type : "post",
			dataType : "json",
			contentType : "application/json;charset=UTF-8",
			data : JSON.stringify({
...
//	var no = $('#sgId').val();
//	$.ajax({
//		url : pathName + "/basicData/downloadUploadFile",
//		type : "post",
//		dataType : "json",
//		contentType : "application/json;charset=UTF-8",
//		data : JSON.stringify({
...
//		success : function(data) {
//			if (data != null) {
//				location.href = pathName + "/basicData/download?no=" + data;
//			}
//		},
//		error : function(data) {
//			
...
function download(_form) {
	var no = $('#sgId').val();
	if(no != null && no != ""){
		_form.action = pathName + "/basicData/download?no="+no;
		_form.submit();
	}else{
		alert("请选中！");
		_form.action = pathName + "/basicData/jump";
		_form.submit();
	}
